...
<template>
    <div class="tmsgBox"  ref="tmsgBox">
                    ...
                    <div class="OwO-body">
                        <ul class="OwO-items">
                            <li class="OwO-item" v-for="(oitem,index) in OwOlist" :key="'oitem'+index" @click="choseEmoji(oitem.title)">
                                <img :src="'static/img/emot/image/'+oitem.url">
                            </li>
                        </ul>
                        <div class="OwO-bar">
        ...
        <div class="tmsg-comments"  ref="listDom">
            <a href="javascript:void(0)" class="tmsg-comments-tip"> 活捉{{commentList?commentList.length:0}} 条</a>
                    ...
                    <li class="tmsg-c-item" v-for="(item,index) in commentList" :key="'common'+index">
                        <article >
                            <header>
                                ...
                                <!-- 服务器获取不到图片信息时，onerror来显示图片 -->
                                <div class="i-name">
                                    {{item.username}}
                                    <!-- commentList=[{avatar:头像，username：用户名，time：时间}] -->
                                </div>
                            ...
                            <section>
                                ...
                                <div  class="tmsg-replay" @click="respondMsg(item.comment_id,item.comment_id)">
                                    回复
                                </div>
                            </section>
                        </article>
...
<script>
    ...
    export default {
        data() { //选项 / 数据
            return {
                pBody:true,//打开表情列表的状态控制  ，每次点击取反
                isRespond:false,//控制  取消回复  按钮的隐藏出现

                textarea: '',//评论文本框中输入内容
                sendTip:'发送',

                leaveId:0,// 用于区分 当前评论是文章评论还是 其他评论,用于区分并向服务器存取数据
                //  0 是文章评论  3 是留言板评论
                ...
                tmsgBox:'',//总评论盒子
                commentList:[],//  评论列表数据 
                //commentList=[{avatar:头像，
                //              username：用户名，
                //             
                //              time：时间,
                //              content,
                //              ChildsSon:{ avatar,username,labbel,time,content
                    //                      
                //                         }这条评论的评论}] -->
                pageId:0,//当前第几页
                
                hasMore:true,  //加载更多按钮显示状态
        ...
        methods: { 
          ...
          choseEmoji(inner){ //选择表情包 ，在输入内容中添加表情
              this.textarea +='[' + inner + ']';
          },
          analyzeEmoji(cont){//编译表情替换成图片在字符串中     // str应该是一个v-html
                var pattern1 = /\[[\u4e00-\u9fa5]+\]/g;
                var pattern2 = /\[[\u4e00-\u9fa5]+\]/;
                var content = cont.match(pattern1);
                var str = cont;
                if(content){ //如果有匹配结果，数组
                    for(var i=0;i<content.length;i++){
                        for(var j=0;j<this.OwOlist.length;j++){
                            if("["+this.OwOlist[j].title +"]" == content[i]){
                                var src = this.OwOlist[j].url;  //解析出原表情
                                break;
                            }
                        }
                        str = str.replace(pattern2,'<img src="static/img/emot/image/'+src+'"/>');  //将原表情替换进去
                             // str应该是一个v-html
                    }
                    // console.log(str);
                }
          ...
          sendMsg(){//发送提交留言
              var that = this;
              if(that.textarea){ //输入的内容不为空
                  that.sendTip = '咻咻咻';
                  if(that.leaveId==0){ // 文章评论
                    //   console.log(that.textarea,that.userId,that.aid,that.leavePid,that.pid);
                      setArticleComment(that.textarea,that.userId,that.aid,that.leavePid,that.pid,function(msg){
                          //未登录时  直接就是data中的userId进行提交，后台将其处理为游客
                        //   console.log(msg);
                          that.textarea = '';
                          that.routeChange();
                          that.removeRespond();
          ...
          respondMsg(leavePid,pid){//回复留言
              ...
              if(localStorage.getItem('userInfo')){
                  var dom = event.currentTarget; //回复按钮
                  dom = dom.parentNode; 
                  this.isRespond = true;  //显示取消回复
                  this.leavePid = leavePid; 
                  this.pid = pid;
                  dom.appendChild(this.$refs.respondBox); // 插入的位置
              }else{ //?????????????????????
          ...
          showCommentList(initData){//评论列表  initData为bool，是否要初始化数据 false即加载更多
              var that = this;
              that.aid =  that.$route.query.aid==undefined  ?  1  :  parseInt(that.$route.query.aid);//获取传参的文章aid,默认第一篇
              //判断当前用户是否登录
              if(localStorage.getItem('userInfo')){ //loaclstorage 有 userInfo  信息表示已登录
                  that.haslogin = true;
                  that.userInfo = JSON.parse(localStorage.getItem('userInfo'));  //找出个人信息
                  that.userId = that.userInfo.userId;  //获取个人id
                //   console.log(that.userInfo);
              }else{
                  that.haslogin = false;
              }
              //从第0页加载数据，还是pageid页
              that.pageId = initData ? 0 : that.pageId; // initData为bool，是否要初始化数据

              //公用设置数据的方法  
              function setData(result){
                  if(result.code==1001){//查询数据 
                  //result={code,data}  直接为服务器端返回的数据
                      var msg = result.data;
                    //   console.log("留言数据",result.data);

                      // 是否显示加载更多按钮
                      if(msg.length>0&&msg.length<8){   //一页八条
                          that.hasMore = false
                      }else{
                          that.hasMore = true;
                      }
                      that.commentList = initData ? msg : that.commentList.concat(msg); 
                      //若要初始化数据 ，则提交查询的pageid=0的信息，并返回，直接放在commentList中覆盖
                      //如果不初始化数据 ，initData=false,则提交查询点击事件后的pageId,并concat在之前的contentList之后

                      that.pageId = msg[msg.length-1].comment_id; //更新pageId
                  }else{//查询数据为空
                      that.hasMore = false; //不显示加载更多
              ...
              if(that.$route.name=='DetailShare'){//文章列表的评论
                  ...
                  if(that.$route.name == 'Message'){//（1：赞赏 2：友情链接 3：留言板 4：关于我）
                      that.leaveId = 3
                  }
...
/* pBody?'OwO':'OwO OwO-open'状态打开后  ，向该元素添加样式 ， 
由于优先级等css特性，
故可以通过.OwO-open .OwO-body 来添加覆盖样式*/
.OwO{
    position: relative;
    z-index: 1;
}
...
.OwO .OwO-logo:hover{  /* 表情按钮hover动画 */
...
.OwO-open .OwO-body{   /* 按钮打开后的 表情展示 */
    display: block;
}
.OwO-open .OwO-logo{  /* 按钮打开后的 按钮 */
    border-radius: 4px 4px 0 0;
    border-bottom: none;
}
.OwO-open .OwO-logo:hover{  /* 按钮打开后去掉动画 */
    animation:none;
...
.OwO .OwO-items .OwO-item{
    ...
    transition:0.3s;/* 作用于全部，这里也就是box-shadow */
    line-height: 19px; /* 给出高度并垂直居中 */
    font-size: 20px;
    cursor: pointer;
}

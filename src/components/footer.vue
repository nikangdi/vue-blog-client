...
<template>
    ...
    <div  class="footBack footBackQian">
        <div class="footer-img" :style="{background:this.$store.state.themeObj.bottom_img?'url('+this.$store.state.themeObj.bottom_img+') no-repeat 50%':footBg}">
        </div>
        <div class="fcontainer">
            ...
            <p>
                © 2019 <a href="#">KangdiBlog</a>. 由 <a href="https://cn.vuejs.org/" target="_blank">Vue</a> 强力驱动. Theme By <a href="https://diygod.me/" target="_blank">diygod.me</a>. 京ICP备17071595号-1.
            </p>
            <p>
                <span>Made with</span> <img class="fheart" src="static/img/heart02.png"/>  <span>by Kangdi1995.</span>
            </p>
        </div>
    </div>
    ...
    export default {
        ...
        methods: { //事件处理器
            runTime:function(){//运行倒计时
                var that = this;
                var oldTime =new Date('2019/04/01 00:00:00');
                var timer = setInterval(function(){
                    var nowTime = new Date();
                    var long_Time = nowTime - oldTime;
                    var days = parseInt(long_Time / 1000 / 60 / 60 / 24 , 10); //计算剩余的天数
                      var hours = parseInt(long_Time / 1000 / 60 / 60 % 24 , 10); //计算剩余的小时
                      var minutes = parseInt(long_Time / 1000 / 60 % 60, 10);//计算剩余的分钟
                      var seconds = parseInt(long_Time / 1000 % 60, 10);//计算剩余的秒数
...
.footBack .fheart{
	...
	animation: heartScale 1.33s ease-in-out infinite; /* 动画无限循环 */	
}

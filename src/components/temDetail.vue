        ...
        <div class="detailBox tcommonBox" >
            ...
            <div class="dshareBox bdsharebuttonbox"  data-tag="share_1">
                ...
                <div class="dlikeColBox">
                    <div class="dlikeBox" @click="likecollectHandle()" >
    ...
    export default {
        data() { //选项 / 数据
            return {
                ...
                //           title: , 
                //           browse_count:浏览次数,
                //           comment_count：评论数，  //****** */ {classId:1,name:'技术分享'},{classId:2,name:'闲言碎语'},{classId:3,name:'事件簿'},{classId:4,name:'创作集'}
                //          class_id:跳转到该类别,
                //           cate_name：类别的名字
                //          content：内容} 

                pdonate:true,//显示赞赏二维码的状态控制,

                likeArt:false,//是否点赞false表示还没赞，true表示已赞
                likeCount:400,//点赞数量
        ...
        methods: { 
            ...
            likecollectHandle(){//用户点击喜欢
                ...
                // if(localStorage.getItem('userInfo')){//判断是否登录
                    ...
                    //     }
                    // }
                    getArtLikeCollect(that.userId,that.aid,function(msg){
                        // console.log('喜欢成功',msg);
                        that.$message({  //显示点赞后的 提示信息！！！！！！！！！！！！！！！！！！！！！！！！！
                             message: tip,
                             type: 'success'
                           });
                    })
                ...
                //           that.$router.push({path:'/Login?login=1'}); //编程式导航
                //      }).catch(() => {//取消关闭弹窗
            ...
            routeChange(){
                var that = this;
                that.aid = that.$route.query.aid==undefined?1:parseInt(that.$route.query.aid); //获取传参的aid，默认为1
                
                //判断用户是否登录
                // if(localStorage.getItem('userInfo')){ 
                //     // that.haslogin = true;  // 取出登录状态
...
.donate-body .donate-item:first-of-type div{//选择出父元素符合该类型的第一个元素，区别first-child
    color:#44b549;
}
